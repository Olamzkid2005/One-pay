...
def send_password_reset(to_email: str, reset_url: str) -> bool:
    ...
    if not to_email or "\n" in to_email or "\r" in to_email:
        logger.error(
            "Invalid email address (header injection attempt): %s", to_email[:50]
        )
        return False
...
def send_invoice_email(
    ...
    # Retry logic with short delays to avoid blocking the HTTP request thread
    max_attempts = 3
    backoff_delays = [1, 2, 4]  # 1s, 2s, 4s — non-blocking for HTTP threads
    ...
    for attempt in range(max_attempts):
        try:
            ...
            msg["Subject"] = (
                f"Invoice {invoice.invoice_number} from {invoice.business_name or 'OnePay'}"
            )
            msg["From"] = Config.MAIL_FROM
            msg["To"] = to_email
...
"""
            ...
            # Send via SMTP
            with smtplib.SMTP(
                Config.MAIL_SERVER, Config.MAIL_PORT, timeout=10
            ) as server:
                if Config.MAIL_USE_TLS:
                    server.starttls()
                if Config.MAIL_USERNAME and Config.MAIL_PASSWORD:
...
def send_merchant_notification_email(
    to_email: str, transaction, invoice, pdf_bytes: Optional[bytes]
) -> bool:
    """
...
def send_2fa_code(to_email: str, code: str) -> bool:
    ...
    try:
        ...
        text_body = f"Your OnePay verification code is: {code}\nThis code will expire in 10 minutes."
        html_body = f"""
        <html>
        <body style="font-family: sans-serif; padding: 20px;">
            <h2>OnePay Authentication</h2>
            <p>Your verification code is:</p>
            <h1 style="font-size: 32px; letter-spacing: 5px; color: #418fff;">{code}</h1>
