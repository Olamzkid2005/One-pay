...
class InvoiceService:
    ...
    def generate_invoice_number(self, db: Session) -> str:
        ...
        for attempt in range(max_retries):
            try:
                ...
                result = db.execute(select(func.max(Invoice.invoice_number))).scalar()
                ...
                logger.warning(
                    "Invoice number generation attempt %d failed: %s", attempt + 1, e
                )
    ...
    def get_invoice_by_number(
        ...
        invoice = (
            db.query(Invoice)
            .filter(
                Invoice.invoice_number == invoice_number, Invoice.user_id == user_id
            )
            .first()
        )
    ...
    def sync_invoice_status(
        self, db: Session, invoice: Invoice, transaction: Transaction
    ) -> None:
        """
