            ...
            with patch("blueprints.public.get_db") as mock_get_db:
                with patch("blueprints.public.check_rate_limit", return_value=True):
                    ...
                    payload = {
                        ...
                        "data": {"reference": "ONEPAY-TEST-123", "status": "success", "amount": 150000},
                    }

                    # Generate valid signature
                    signature = self._generate_valid_signature(payload["data"], "webhook-secret-32-chars-long-test")

                    with patch("services.webhook.sync_invoice_on_transaction_update"):
                        response = client.post(
                            "/api/webhooks/korapay",
                            data=json.dumps(payload),
                            headers={"Content-Type": "application/json", "x-korapay-signature": signature},
                        )
        ...
        with patch("blueprints.public.get_db") as mock_get_db:
            with patch("blueprints.public.check_rate_limit", return_value=True):
                ...
                response = client.post(
                    "/api/webhooks/korapay",
                    data=json.dumps(payload),
                    headers={"Content-Type": "application/json", "x-korapay-signature": invalid_signature},
                )
    ...
    def test_webhook_with_missing_signature_returns_401(self, client):
        ...
        with patch("blueprints.public.get_db") as mock_get_db:
            with patch("blueprints.public.check_rate_limit", return_value=True):
                ...
                # No signature header
                response = client.post(
                    "/api/webhooks/korapay", data=json.dumps(payload), headers={"Content-Type": "application/json"}
                )
        ...
        with patch("blueprints.public.get_db") as mock_get_db:
            with patch("blueprints.public.check_rate_limit", return_value=True):
                ...
                response = client.post(
                    "/api/webhooks/korapay",
                    data="invalid json {",
                    headers={"Content-Type": "application/json", "x-korapay-signature": "0" * 64},
                )
