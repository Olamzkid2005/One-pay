...
def test_korapay_webhook_forwards_to_voicepay(client, app, monkeypatch):
    ...
    def mock_send_voicepay_webhook(payload, webhook_url, secret, **kwargs):
        voicepay_calls.append({"payload": payload, "webhook_url": webhook_url, "secret": secret})
        return {"success": True, "status_code": 200, "tx_ref": payload["tx_ref"]}

    monkeypatch.setattr("services.voicepay_webhook.send_voicepay_webhook", mock_send_voicepay_webhook)
    ...
    korapay_payload = {
        ...
        "data": {"reference": "VP-BILL-123-1234567890", "status": "success", "amount": 9000},
    }

    # Generate KoraPay signature
    data_bytes = json.dumps(korapay_payload["data"], separators=(",", ":")).encode()
    korapay_signature = hmac.new(Config.KORAPAY_WEBHOOK_SECRET.encode(), data_bytes, hashlib.sha256).hexdigest()

    # Send webhook
    response = client.post(
        "/api/webhooks/korapay", json=korapay_payload, headers={"x-korapay-signature": korapay_signature}
    )
...
def test_non_voicepay_transaction_no_webhook(client, app, monkeypatch):
    ...
    korapay_payload = {
        "event": "charge.success",
        "data": {"reference": "ONEPAY-REGULAR-123", "status": "success", "amount": 5000},
    }
...
def test_voicepay_webhook_disabled_no_forwarding(client, app, monkeypatch):
    ...
    korapay_payload = {
        "event": "charge.success",
        "data": {"reference": "VP-BILL-DISABLED-123", "status": "success", "amount": 9000},
    }
